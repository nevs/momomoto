...
class TestReal < Test::Unit::TestCase
  ...
  def test_default
    ...
    a = Momomoto::Datatype::Real.new( row )
    assert_equal( 'default-value', a.default )
    assert_equal( false, Momomoto::Datatype::Real.new.default )
  end

  def test_not_null
    row = Momomoto::Information_schema::Columns.create
    row.is_nullable = "YES"
    assert_equal( false, Momomoto::Datatype::Real.new( row ).not_null? )
    row.is_nullable = "NO"
    assert_equal( true, Momomoto::Datatype::Real.new( row ).not_null? )
    row.is_nullable = nil
    assert_equal( false, Momomoto::Datatype::Real.new( row ).not_null? )
  end

  def test_operator_sign
    dt = Momomoto::Datatype::Real
