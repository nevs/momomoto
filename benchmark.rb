...
Benchmark.benchmark(" "*40 + CAPTION, 40, FMTSTR, ">total:", ">avg:") do | b |
  ...
  times << b.report("querying #{QUERY_REPEAT} times for subset") do
    QUERY_REPEAT.times do a = Momomoto::Information_schema::Columns.select({:table_name => 'columns'}) end
  end
...
Benchmark.benchmark(" "*40 + CAPTION, 40, FMTSTR, ">total:", ">avg:") do | b |
  ...
  times << b.report("querying #{QUERY_REPEAT} times for subset") do
    QUERY_REPEAT.times do a = Columns.find_all_by_table_name('columns') end
  end
